...
// RUN:   --iree-vm-conversion --canonicalize --mlir-print-local-scope %s | FileCheck %s
...
func.func @parameterGather(%device: !hal.device, %queue_affinity: i64, %wait: !hal.fence, %signal: !hal.fence, %target_buffer: !hal.buffer) {
  ...
  //  CHECK-DAG: %[[KEY_TABLE:.+]] = vm.rodata.inline : !vm.buffer = #util.composite<36xi8, [
  // CHECK-NEXT:  dense<[0, 4, 4, 4, 8, 4]> : vector<6xi32>,
  // CHECK-NEXT:  dense<[107, 101, 121, 48, 107, 101, 121, 49, 107, 101, 121, 50]> : vector<12xi8>,
  // CHECK-NEXT: ]>
  ...
  // CHECK-SAME: (%[[DEVICE]], %[[QUEUE_AFFINITY]], %[[WAIT]], %[[SIGNAL]],
  // CHECK-SAME:   %[[SCOPE]], %[[TARGET_BUFFER]], %[[KEY_TABLE]], %[[SPANS]])
...
func.func @parameterScatter(%device: !hal.device, %queue_affinity: i64, %wait: !hal.fence, %signal: !hal.fence, %source_buffer: !hal.buffer) {
  ...
  // CHECK-SAME: (%[[DEVICE]], %[[QUEUE_AFFINITY]], %[[WAIT]], %[[SIGNAL]],
  // CHECK-SAME:   %[[SCOPE]], %[[SOURCE_BUFFER]], %[[KEY_TABLE]], %[[SPANS]])

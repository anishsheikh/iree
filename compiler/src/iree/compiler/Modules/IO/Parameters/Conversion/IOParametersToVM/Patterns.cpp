...
static Value buildKeyTable(Location loc, ArrayAttr keysAttr,
                           OpBuilder &builder) {
  SmallVector<int32_t> table;
  SmallVector<int8_t> data;
  for (auto key : keysAttr.getAsValueRange<StringAttr>()) {
    table.push_back(data.size());
    table.push_back(key.size());
    data.append(key.begin(), key.end());
  }
  return builder.create<IREE::VM::RodataInlineOp>(
      loc, IREE::VM::RefType::get(builder.getType<IREE::VM::BufferType>()),
      IREE::Util::CompositeAttr::get(
          builder.getContext(),
          {
...
struct ScatterOpConversion
  ...
  matchAndRewrite(IREE::IO::Parameters::ScatterOp scatterOp, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    auto keyTable = buildKeyTable(scatterOp.getLoc(),
                                  adaptor.getTargetKeysAttr(), rewriter);
    auto spans = buildIndirectSpans(

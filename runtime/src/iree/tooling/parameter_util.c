...
IREE_FLAG_LIST(
    string, parameters,
    "Specifies a parameter file to make available to programs with a named\n"
    "scope like `my_scope=some_file.npy`. Each occurrence of the flag defines\n"
    "a new parameter scope and must have a unique name. In certain cases the\n"
    "scope name can be omitted but only one such anonymous parameter file is\n"
    "allowed.\n"
    "\n"
    "Supported formats:\n"
...
static iree_status_t iree_io_append_parameter_file_to_index(
  ...
  if (iree_string_view_equal_case(path_ext, IREE_SV("safetensors"))) {
...
iree_status_t iree_tooling_create_parameters_module_from_flags(
  ...
  for (iree_host_size_t i = 0; i < FLAG_parameters_list().count; ++i) {
    ...
    if (iree_string_view_split(flag, '=', &scope, &path) ==
